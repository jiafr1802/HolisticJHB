...
class CONFIG(object):
    ...
    def __init__(self, parser):
        ...
        if mode in ['test', 'demo']:
            ...
            weight = os.path.join(log_path, 'model_best.pth')#需要加载的已经训练好的网络的路径
            #看起来我们是要让我们的模型的路径和log_path/model_best.pth的路径一致，没关系，不用着急改，先标注一下
            #welcome
...
def mount_external_config(cfg):
    if cfg.config['data']['dataset'] == '3d-future':
        dataset_config = Data_Config('3d-future')
        ...
        print("in config_utils.py: mount_external_config: cfg.dataset_config: ", cfg.dataset_config)
