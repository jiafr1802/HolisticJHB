...
def train_epoch(cfg, epoch, trainer, dataloaders):
    ...
    for phase in ['train', 'test']:
        ...
        for iter, data in enumerate(dataloader):
            ...
            '''
            if phase == 'train':
                loss = trainer.train_step(data)#推测train_step也是一个epoch的训练
                #models/trainning.py 中Trainer类中的方法，这个方法继承基类

            else:#test
                loss = trainer.eval_step(data)
                #models/total3d/trainning.py 中定义
        ...
        cfg.log_string('=' * 100)
        for loss_name, loss_value in loss_recorder.loss_recorder.items():#就是我们看到的一边训练一边输出的情况
...
def train(cfg, trainer, scheduler, checkpoint, train_loader, test_loader):
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        trainer.show_lr()
        start = time()
        eval_loss_recorder = train_epoch(cfg, epoch + 1, trainer, dataloaders)#train_epoch是一个epoch的训练
