...
class RelationNet(nn.Module):
    ...
    def forward(self, a_features, g_features, split, rel_pair_counts):
        ...
        '''
        因为g_weights 从这里开始就是用全连接层 n x 64 -> n x 16 而 n x 16 意味着16个head
        那么我们想要的是 n x 1
        那么即使不改变也是得到的是 n x  64 
        那么更加不行。
        唯一一个方法是设计一个全连接层 64 -> 
        '''
        ...
        obj_num = split[-1][-1]#total num in batch
        weight_matrix=torch.zeros(obj_num,obj_num).cuda()#jfr
        for interval_idx, interval in enumerate(split):#jfr
            sample_k_features = k_features[interval[0]:interval[1], :]# num_objects_in_batch x dim
            sample_q_features = q_features[interval[0]:interval[1], :]#Only the
            sample_a_weights = torch.div(torch.mm(sample_k_features, sample_q_features.transpose(0, 1)), math.sqrt(rel_cfg.d_k * rel_cfg.Nr))
            '''
            / 1024 yeah
            因为这里理论上应该是我们做向量点乘的俩向量的维度（可以回顾论文的 3. Object Relation Module 部分的公式4）
            因为后面head = 16 -> dim of vec = 64
            目前 dim of vec = 64 
            '''
            #jfr transpose(1,2)-> transpose(0,1)
            #其实 div()后面除以什么都没关系，因为是所有等比的改变，所以这里无所谓了
            #jfr torch.bmm不用吧～我觉得不用啦。
            fin_weight = self.softmax(sample_a_weights)
            weight_matrix[interval[0]:interval[1],interval[0]:interval[1]]=fin_weight#jfr

        # divided by batch and relational group
        # Nr x num_objects_in_batch x dim
        k_features = k_features.view(-1, rel_cfg.Nr, rel_cfg.d_k).transpose(0, 1) #N_r=16
        q_features = q_features.view(-1, rel_cfg.Nr, rel_cfg.d_k).transpose(0, 1) # d_k = 64
        ...
        for interval_idx, interval in enumerate(split):
            ...
            #print("sample_g_weights.shape: ",sample_g_weights.shape)
            #print("sample_a_weights.shape: ",sample_a_weights.shape)
        ...
        return r_features, weight_matrix